...
class LTIRepository implements ILTIRepository
{
    ...
    /**
     * Returns a random string of the appropriate length to use as the consumer
     * key in LTI credentials
     *
     * @return string
     */
    ...
    /**
     * Returns a random string of the appropriate length to use as the
     * secret key in LTI credentials
     *
     * @return string
     */
    ...
    /**
     * When we get the resource link id from the LMS
     * for our activity, this adds it to the database.
     *
     ...
     * the first time we see the activity id in a request. Thereafter,
    ...
    public function handleActivityLaunchRequest(LTIRequest $request, Activity $activity)
    {
        ...
        $resourceLink = $this->getResourceLinkFromRequest($request, $activity);

        //We verify that the oath signature on the incoming post
        //request is valid
        $authenticator = AuthenticatorFactory::make($request);
        //If there's a problem, this will throw an InvalidLTILogin exception
